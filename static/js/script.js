...
function updateStats() {
    ...
    document.getElementById('complimentCount').textContent = complimentCount;
...
async function generateCompliment() {
    try {
        ...
        // Запасной комплимент
        const fallbackCompliments = ["Красава!", "Молодец!", "Лучший!"];
...
function generateSpecialCompliment(type) {
    const specialCompliments = {
        rainbow: ["🌈 Радужная звезда!", "✨ Сияешь всеми цветами!", "🎨 Живописный гений!"],
        fire: ["🔥 Огненная легенда!", "🔥 Горишь ярче всех!", "🔥 Неукротимая энергия!"],
        gold: ["💰 Золотой стандарт!", "🏆 Бесценный друг!", "👑 Королевская особа!"]
    };
